 ...
 */
export async function createEpisodePost(reddit, subredditName, title, body) {
 ...
 */
export async function applyEpisodeFlair(reddit, subredditName, postId) {
    try {
        const flairs = await reddit.getPostFlairTemplates(subredditName);
        const episodeFlair = flairs.find((f) => (f.text ?? '').toLowerCase().trim() === 'episode discussion');
        if (!episodeFlair) {
            console.warn('[postManager] "Episode Discussion" flair template not found on subreddit. ' +
 ...
 */
export async function managePins(reddit, redis, newPostId) {
    ...
    if (previousPostId && previousPostId !== newPostId) {
        try {
            ...
            console.warn(`[postManager] Could not unpin previous post ${previousPostId}:`, err);
        }
    }
